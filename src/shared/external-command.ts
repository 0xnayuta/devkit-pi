...
export interface ExternalCommandRunner {
	isAvailable(command: ExternalCommandSpec, options?: ExternalCommandResolveOptions): Promise<boolean>;
	run(command: ExternalCommandSpec, options: ExternalCommandRunOptions): Promise<ExternalCommandResult>;
}
...
function searchPath(executable: string, env: NodeJS.ProcessEnv, extraSearchPaths: string[] = []): string | undefined {
...
export class NodeExternalCommandRunner implements ExternalCommandRunner {
	async isAvailable(command: ExternalCommandSpec, options: ExternalCommandResolveOptions = {}): Promise<boolean> {
