...
export function createDevkitRuntime(pi: ExtensionAPI, options: CreateDevkitRuntimeOptions = {}): DevkitRuntime {
	const baseLogger =
		options.logger ??
		createLogger({
			module: "extension",
	...
	const registerModules = () => {
		...
		const subagentsConfig = {
			...
			allowLspTools:
				effectiveConfig.subagents.allowLspTools && effectiveConfig.lsp.enabled && effectiveConfig.lsp.tool.enabled,
		};
	...
	return {
		...
		async dispose(): Promise<void> {
			try {
				...
				logger.error("runtime.dispose_failed", "Failed to dispose runtime resources", undefined, error);
			}
		},
	};
}
