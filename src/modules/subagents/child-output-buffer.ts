...
export class ChildStdoutCollector {
	...
	private boundUnterminatedLine(): void {
		...
		const limit = looksLikeJsonlCandidate(this.lineBuffer) ? this.maxUnterminatedJsonlBytes : this.maxStdoutBytes;
...
export function appendLimitedOutput(buffer: LimitedOutputBuffer, chunk: Buffer, maxBytes: number): Buffer {
