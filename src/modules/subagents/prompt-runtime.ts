...
const SUBAGENT_ORCHESTRATION_SKILL_NAME_PATTERN = /<name>\s*devkit-pi\s*<\/name>/;
...
export function buildChildPrompt(options: BuildChildPromptOptions): string {
	const rolePrompt =
		options.agentSystemPrompt?.trim() ||
	...
	const depthInstruction =
		options.childDepth !== undefined && options.maxDepth !== undefined
...
export function stripSubagentOrchestrationSkill(prompt: string): string {
	return prompt
		.replace(/\n{0,2}<skill\s+name=["']devkit-pi["'][^>]*>[\s\S]*?<\/skill>\n{0,2}/g, "\n\n")
