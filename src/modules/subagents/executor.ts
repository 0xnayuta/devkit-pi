...
function loadAgent(agentName: string, cwd: string, deps: ExecutorDeps): AgentToolResult<Details> | AgentConfig {
...
export function filterToolsForReadonly(agent: AgentConfig, config: ResolvedSubagentsConfig): string[] {
...
function getUiConfirm(ui: ExtensionContext["ui"] | undefined): ((message: string) => Promise<boolean>) | null {
	const candidate = (ui as unknown as { confirm?: unknown } | undefined)?.confirm;
	return typeof candidate === "function"
		? (message: string) =>
				Promise.resolve((candidate as (message: string) => boolean | Promise<boolean>)(message)).then(Boolean)
		: null;
}
...
async function shouldAllowProjectAgentExecution(
	...
	if (ctx.hasUI && confirm) {
		return confirm(`Allow execution of project-local agent '${agent.name}' from ${agent.filePath}?`);
	}
...
function formatSubagentFailure(input: {
	...
	const partial = input.partialOutput && input.partialOutput !== input.error ? input.partialOutput : undefined;
	const fallback = !partial && input.output && input.output !== input.error ? input.output : undefined;
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt++) {
			...
			sessionFile = path.join(sessionDir, attempt === 1 ? "session.jsonl" : `session-attempt-${attempt}.jsonl`);
			...
			for (const jsonStreamProfile of jsonStreamProfiles) {
				...
				try {
					...
					if (result.outputLimitExceeded) {
						output = result.output || "Subagent child output exceeded the configured hard limit and was stopped.";
		...
		let truncationError: { code: typeof SUBAGENT_ERROR_CODES.SUBAGENT_OUTPUT_TRUNCATED; message: string } | undefined;
...
): Array<{ type: "text"; text: string } | { type: "toolCall"; name: string; args: Record<string, unknown> }> {
	const items: Array<
