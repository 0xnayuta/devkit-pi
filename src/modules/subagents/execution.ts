...
export async function runSync(cwd: string, args: string[], options: RunSyncOptions = {}): Promise<RunSyncResult> {
	const {
		signal,
		timeoutMs,
		idleTimeoutMs,
		...
		maxUnterminatedJsonlBytes = Math.max(maxStdoutBytes, DEFAULT_SUBAGENT_MAX_UNTERMINATED_JSONL_BYTES),
		spawnCommand,
	} = options;
	...
	return new Promise((resolve) => {
		...
		child.on("close", (code, closeSignal) => {
			...
			const collected = outputLimitExceeded
				? {
						output: outputLimitMessage ?? "Subagent child output exceeded a hard limit and was stopped.",
						usage: undefined,
