...
export function formatLogs(options: LogsOptions = {}): string {
	...
	if (entries.length === 0) {
		...
		for (const entry of entries.reverse()) {
			...
			const duration = entry.duration !== undefined ? `${entry.duration}ms`.padEnd(7) : "".padEnd(7);
			const statusIcon =
	...
	if (Object.keys(stats.providerStats).length > 0) {
		...
		for (const [name, providerStats] of Object.entries(stats.providerStats)) {
			...
			lines.push(`    ${name.padEnd(10)} ${providerStats.requests} requests, ${successRate}% success`);
		}
	}
