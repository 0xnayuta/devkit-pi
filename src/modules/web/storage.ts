...
import type { ExtractedContent, GetSearchContentInput, QueryResultData, StoredResult, WebToolError } from "./types.ts";
...
export function setSessionResultAppender(handler: ((data: PersistedStoredResult) => void) | null): void {
	appendEntry = handler;
}

export function setStorageLimits(options: { maxStoredResults: number; maxStoredContentChars: number }): void {
...
function toPersistedStoredResult(id: string, envelope: StoredEnvelope): PersistedStoredResult {
	return {
		...
		...(envelope.result.type === "fetch" ? { urls: envelope.result.urls } : { queries: envelope.result.queries }),
	};
}
...
export function restoreResultsFromSession(branch: unknown[], now = Date.now(), ttlMs = WEB_RESULTS_TTL_MS): number {
	results.clear();
	let restored = 0;
...
function truncateText(value: string, maxContentChars: number): { content: string; truncated: boolean } {
...
function limitExtractedContent(result: ExtractedContent, maxContentChars: number): ExtractedContent {
...
function selectFetchResult(
	...
	if (typeof params.urlIndex === "number") {
		if (!Number.isInteger(params.urlIndex) || params.urlIndex < 0 || params.urlIndex >= stored.length) {
			return error(
	...
	if (params.url) {
		...
		if (!matched) {
			return error(WEB_ERROR_CODES.NOT_FOUND, `URL "${params.url}" not found. ${formatFetchSelectorHint(stored)}`);
		}
		return matched;
	}
...
function selectSearchResult(
	...
	if (typeof params.queryIndex === "number") {
		if (!Number.isInteger(params.queryIndex) || params.queryIndex < 0 || params.queryIndex >= stored.length) {
			return error(
				WEB_ERROR_CODES.NOT_FOUND,
...
export function getSearchContent(params: GetSearchContentInput, maxContentChars: number): GetSearchContentResult {
	...
	if (!stored) {
		return error(WEB_ERROR_CODES.NOT_FOUND, `No stored web result found for responseId: ${responseId}`);
	}
