...
export const SEARCH_PROVIDER_NAMES = Object.keys(SEARCH_PROVIDER_METADATA) as WebSearchProviderName[];
...
export function getProviderDisplayName(name: string): string {
	return name in SEARCH_PROVIDER_METADATA ? SEARCH_PROVIDER_METADATA[name as WebSearchProviderName].displayName : name;
}
...
export function getProviderApiKeyEnv(config: ResolvedWebConfig, name: WebSearchProviderName): string | undefined {
