...
function normalizeNumResults(params: WebSearchInput, config: ResolvedWebConfig): number {
	const requested = params.numResults;
	if (typeof requested !== "number" || !Number.isFinite(requested)) {
		return config.maxResults;
	}
	return Math.min(Math.max(Math.floor(requested), 1), config.maxResults);
}
...
function classifySearchError(err: unknown, providerName: string, startTs: number): WebToolError {
	...
	if (isAbortLikeError(err)) {
		...
		return {
			error: {
				...
				message:
					"web_search timed out or was aborted. Try fewer queries, smaller numResults, or increase web.timeoutMs.",
			},
		};
	}
	...
	if (message.includes("is required for web_search provider") || errStatus === 401 || errStatus === 403) {
		const status = errStatus ?? 403;
		const webError = mapHttpStatusToError(status, providerName, `${displayName} authentication failed`);
	...
	if (errStatus === 429) {
		recordSearchActivity(providerName, "rate_limited", startTs, WEB_ERROR_CODES.PROVIDER_RATE_LIMITED);
		return {
			error: {
	...
	if (lower.includes("fetch failed") || lower.includes("enotfound") || lower.includes("econn")) {
		const webError = mapNetworkErrorToWebError(err instanceof Error ? err : new Error(message), providerName);
		recordSearchActivity(providerName, "error", startTs, webError.code);
		return {
			error: {
				code: webError.code,
...
export async function webSearch(
	...
	if (queries.length === 0) {
		return error(WEB_ERROR_CODES.WEB_SEARCH_INVALID_QUERY, "web_search requires a non-empty query or queries");
	}
	...
	for (const provider of selection.providers) {
		...
		try {
			...
			return {
				responseId,
				queries: limitSearchOutput(queryResults, config.maxContentChars),
			};
		} catch (err) {
	...
	return lastError ?? error(WEB_ERROR_CODES.WEB_SEARCH_FAILED, "No web_search provider completed successfully.");
}
