...
function detectSupportedContent(
  ...
  if (isBinaryContent(body)) {
    return {
      ...
      source: "magic-bytes",
      unsupportedReason:
        "Binary content detected (magic bytes). Only text-based content is supported",
    };
  }
...
export async function fetchUrlContent(
  ...
  if (detected.type === "html" && config.enableJinaFallback) {
    ...
    if (shouldUseJina && !isPrivateNetworkUrl(finalUrl)) {
      const jina = await fetchFromJinaReader(
        finalUrl,
        config.jinaTimeoutMs,
        limits.maxResponseBytes,
        signal
      );
      if (jina) {
...
export async function fetchContent(
  ...
  try {
    ...
    for (const url of urls) {
      storedResults.push(
        await withThrottle(() => fetchUrlContent(url, config, signal, params.preferReader))
      );
    }
