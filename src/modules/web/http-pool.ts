...
export class HttpConnectionPool {
	...
	getStats(): PoolStats {
		return {
			...
			failureRate: this.stats.totalRequests > 0 ? this.stats.failedRequests / this.stats.totalRequests : 0,
		};
	}

	private getActiveSocketCount(): number {
		// Approximate based on requests
		const approxActive = Math.min(this.stats.totalRequests % this.config.maxSockets, this.config.maxSockets);
		return approxActive;
	}
	...
	updateConfig(config: Partial<ConnectionPoolConfig>): void {
		...
		if (newConfig.maxSockets !== this.config.maxSockets || newConfig.maxFreeSockets !== this.config.maxFreeSockets) {
			this.destroy();
