...
export function createWebError(
	...
	return {
		...
		recovery,
		retryable: options?.retryable ?? (recovery?.action === "retry" || recovery?.action === "fallback"),
	};
}
...
export function mapHttpStatusToError(status: number, provider?: string, message?: string): WebError {
	switch (status) {
		case 401:
		case 403:
			return createWebError(WEB_ERROR_CODES.PROVIDER_AUTH_FAILED, message ?? "Authentication failed", {
				provider,
				retryable: false,
			});

		case 429:
			return createWebError(WEB_ERROR_CODES.PROVIDER_RATE_LIMITED, message ?? "Rate limit exceeded", {
				provider,
				retryable: true,
			});
		...
		case 503:
		case 504:
			return createWebError(WEB_ERROR_CODES.PROVIDER_UNAVAILABLE, message ?? "Provider unavailable", {
				provider,
				retryable: true,
			});
