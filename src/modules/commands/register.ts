...
function getToolkitArgumentCompletions(argumentPrefix: string): AutocompleteItem[] | null {
	...
	const filtered = TOOLKIT_SUBCOMMAND_COMPLETIONS.filter((item) => item.value.startsWith(normalized));
	...
	pi.registerCommand("toolkit", {
		...
		handler: async (args: string, ctx) => {
			...
			try {
				...
				ctx.ui.notify(`Toolkit command failed: ${error instanceof Error ? error.message : error}`, "error");
			}
		},
	});
}
