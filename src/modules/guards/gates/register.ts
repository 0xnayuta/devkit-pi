...
import type { GuardsBlockMode, GuardsMode, GuardsNonInteractivePolicy } from "../../../shared/types.ts";
...
function getUiConfirm(ctx: ExtensionContext): ((message: string) => boolean | Promise<boolean>) | null {
	...
	return typeof candidate === "function" ? (candidate as (message: string) => boolean | Promise<boolean>) : null;
}

function toDeniedDecision(blockMode: GuardsBlockMode): Extract<GateDecision, "deny_soft" | "deny_hard"> {
...
function formatBlockDecisionNotice(toolName: string, blockMode: GuardsBlockMode, decision: GateDecision): string {
	return `[devkit-pi] Guard mode=block (${blockMode}): potential write tool '${toolName}' decision=${decision}.`;
}

function enforceHardBlock(decision: GateDecision, mode: "confirm" | "block", toolName: string, logger: Logger): void {
...
export function registerGuardsGateHandlers(
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (options.mode === "confirm") {
			...
			notify(ctx, formatNonInteractiveDecisionNotice(toolName, options.nonInteractivePolicy, decision));
			enforceHardBlock(decision, "confirm", toolName, options.logger);
			return;
		}
