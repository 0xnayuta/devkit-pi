...
export interface DiagnosticsCycleParams {
	...
	isNew: boolean;
	evict?: boolean;
	waitForDiagnostics(client: LSPClient, absPath: string, timeoutMs: number, isNew: boolean): Promise<boolean>;
	openOrUpdate(
		clients: LSPClient[],
		absPath: string,
		uri: string,
...
export function collectClientDiagnosticsForPath(clients: LSPClient[], absPath: string): Diagnostic[] {
