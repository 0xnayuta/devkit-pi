...
function normalizeExecuteArgs(
  ...
  if (isContextLike(ctxArg)) {
    return {
      ...
      onUpdate:
        typeof onUpdateArg === "function" ? (onUpdateArg as ExecuteArgs["onUpdate"]) : undefined,
      ctx: ctxArg,
    };
  }
...
function formatSignature(help: any): string {
  ...
  if (sig.parameters?.length) {
    const params = sig.parameters
      .map((p: any) =>
        typeof p.label === "string" ? p.label : Array.isArray(p.label) ? p.label.join("-") : ""
      )
      .filter(Boolean);
  ...
  pi.registerTool(
    defineTool({
      ...
      renderCall(args, theme) {
        ...
        let text =
          theme.fg("toolTitle", theme.bold("lsp ")) + theme.fg("accent", params.action || "...");
