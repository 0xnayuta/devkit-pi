...
import { type Diagnostic, InitializedNotification, InitializeRequest } from "vscode-languageserver-protocol";
...
export async function initializeSpawnedClient(params: InitializeClientParams): Promise<LSPClient> {
	const { handle, root, serverId, initTimeoutMs, normalizeFsPath, onClientClosed, onClientProcessError } = params;
	...
	conn.onNotification("textDocument/publishDiagnostics", (payload: { uri: string; diagnostics: Diagnostic[] }) => {
...
export async function initClientWithSpawn(params: InitClientWithSpawnParams): Promise<LSPClient | undefined> {
	const {
		root,
		serverId,
		initTimeoutMs,
