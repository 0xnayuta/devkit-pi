...
function asHookToolEvent(event: unknown): HookToolEvent {
	return event && typeof event === "object" ? (event as HookToolEvent) : {};
}
...
export function registerLspHook(pi: ExtensionAPI, config: RequiredLspHookConfig): void {
	...
	pi.on("tool_call", async (event, ctx) => {
		const toolEvent = asHookToolEvent(event);
		const input = inputRecord(toolEvent.input);
		clearIdleShutdownTimer();

		if (toolEvent.toolName === "lsp") {
		...
		if (toolEvent.toolName !== "read" && toolEvent.toolName !== "write" && toolEvent.toolName !== "edit") return;
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		if (toolEvent.toolName !== "write" && toolEvent.toolName !== "edit") return;
		...
		if (hookMode === "agent_end") {
			const includeWarnings = toolEvent.toolName === "write";
		...
		const content = Array.isArray(toolEvent.content) ? toolEvent.content : [];
		return {
