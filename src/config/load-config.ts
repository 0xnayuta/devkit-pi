...
export function getConfigPath(): string {
	return path.join(process.env.HOME ?? os.homedir(), ".pi", "agent", "extensions", "devkit-pi", "config.json");
}
...
function normalizeProviderPriority(value: unknown): ResolvedWebConfig["providerPriority"] {
	...
	const filtered = value.filter(
		(item): item is Exclude<WebSearchProviderName, "auto"> =>
			typeof item === "string" && PROVIDER_NAMES.includes(item as Exclude<WebSearchProviderName, "auto">)
	);
...
function normalizeWebConfig(base: WebConfig | undefined): ResolvedWebConfig {
	return {
		...
		enableJinaFallback: booleanValue(base?.enableJinaFallback, DEFAULT_WEB_CONFIG.enableJinaFallback),
		...
		maxStoredContentChars: positiveInteger(base?.maxStoredContentChars, DEFAULT_WEB_CONFIG.maxStoredContentChars),
		allowPrivateNetwork: booleanValue(base?.allowPrivateNetwork, DEFAULT_WEB_CONFIG.allowPrivateNetwork),
		...
		searxng: {
			...
			defaultEngine: nonEmptyString(base?.searxng?.defaultEngine, DEFAULT_WEB_CONFIG.searxng.defaultEngine),
		},
		brave: {
...
function normalizeLspReadonlyActions(value: unknown): LspReadonlyAction[] {
	...
	const filtered = value.filter(
		(item): item is LspReadonlyAction => typeof item === "string" && allowed.has(item as LspReadonlyAction)
	);
	return [...new Set(filtered)];
}

function normalizeSubagentsProjectAgentPolicy(value: unknown): ResolvedSubagentsConfig["projectAgentPolicy"] {
	return value === "allow" || value === "confirm" ? value : DEFAULT_SUBAGENTS_CONFIG.projectAgentPolicy;
}

function normalizeSubagentsNonInteractivePolicy(value: unknown): ResolvedSubagentsConfig["nonInteractivePolicy"] {
	return value === "allow" || value === "deny" ? value : DEFAULT_SUBAGENTS_CONFIG.nonInteractivePolicy;
}
...
function normalizeSubagentsConfig(base: SubagentsConfig | undefined): ResolvedSubagentsConfig {
	return {
		...
		retry: {
			...
			maxAttempts: positiveInteger(base?.retry?.maxAttempts, DEFAULT_SUBAGENTS_CONFIG.retry.maxAttempts),
		},
	};
}
...
function normalizeCommandsConfig(base: CommandsConfig | undefined): ResolvedToolkitConfig["commands"] {
	return {
...
function normalizeGuardsNonInteractivePolicy(value: unknown): ResolvedToolkitConfig["guards"]["nonInteractivePolicy"] {
...
function normalizeGuardsBlockMode(value: unknown): ResolvedToolkitConfig["guards"]["blockMode"] {
	return value === "preview" || value === "soft" || value === "hard" ? value : DEFAULT_GUARDS_CONFIG.blockMode;
}
...
function normalizeGuardsConfig(base: GuardsConfig | undefined): ResolvedToolkitConfig["guards"] {
	return {
		...
		firstWriteReminder: booleanValue(base?.firstWriteReminder, DEFAULT_GUARDS_CONFIG.firstWriteReminder),
		verificationReminder: booleanValue(base?.verificationReminder, DEFAULT_GUARDS_CONFIG.verificationReminder),
	};
}

function normalizeConvertContentConfig(base: ConvertContentConfig | undefined): ResolvedConvertContentConfig {
	return {
		...
		maxResponseBytes: positiveInteger(base?.maxResponseBytes, DEFAULT_CONVERT_CONTENT_CONFIG.maxResponseBytes),
		maxContentChars: positiveInteger(base?.maxContentChars, DEFAULT_CONVERT_CONTENT_CONFIG.maxContentChars),
		allowPrivateNetwork: booleanValue(base?.allowPrivateNetwork, DEFAULT_CONVERT_CONTENT_CONFIG.allowPrivateNetwork),
	};
}
