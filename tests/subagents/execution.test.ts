...
`);
	...
	it("project-local agent confirm gate denies in non-interactive mode when policy=deny", async () => {
		const config = mergeConfig({
			subagents: {
		...
		const executor = createSubagentExecutor({
			...
			config,
			getSubagentSessionRoot: () => {
				const dir = fs.mkdtempSync(path.join(os.tmpdir(), "devkit-pi-project-gate-"));
				tempDirs.push(dir);
				return dir;
			},
			discoverAgents: () => ({
			...
			runSyncImpl: async () => {
				runSyncCalled = true;
				return { exitCode: 0, output: "ok", final: true };
			},
		});
		...
		assert.equal(result.details.error?.code, SUBAGENT_ERROR_CODES.SUBAGENT_DISABLED);
		assert.match(result.details.error?.message ?? "", /non-interactive policy/);
		assert.equal(result.details.results.length, 0);
	});

	it("project-local agent confirm gate uses interactive confirm result", async () => {
		const config = mergeConfig({
			subagents: {
				projectAgentPolicy: "confirm",
				nonInteractivePolicy: "deny",
		...
		const executor = createSubagentExecutor({
			...
			config,
			getSubagentSessionRoot: () => {
				const dir = fs.mkdtempSync(path.join(os.tmpdir(), "devkit-pi-project-gate-ui-"));
				tempDirs.push(dir);
				return dir;
			},
			discoverAgents: () => ({
		...
		const denied = await executor.execute(
			"project-gate-ui-deny",
			{ agent: "project-agent-ui", task: "run" },
			new AbortController().signal,
			undefined,
			{
				cwd: process.cwd(),
