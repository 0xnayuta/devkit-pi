...
import { collectSymbols, filterDiagnosticsBySeverity, formatDiagnostic } from "../../src/modules/lsp/formatters.ts";
...
describe("lsp formatters", () => {
	...
	it("filters diagnostics by severity", () => {
		...
		assert.deepEqual(
			filterDiagnosticsBySeverity(diags, "error").map((d) => d.message),
			["error"]
		);
		assert.deepEqual(
			filterDiagnosticsBySeverity(diags, "warning").map((d) => d.message),
			["error", "warning"]
		);
		assert.deepEqual(
			filterDiagnosticsBySeverity(diags, "info").map((d) => d.message),
			["error", "warning", "info"]
		);
		assert.deepEqual(
			filterDiagnosticsBySeverity(diags, "hint").map((d) => d.message),
	...
	it("collects symbols with depth, query, and selectionRange fallback", () => {
		...
		assert.deepEqual(allLines, ["RootClass (1:7)", "  methodOne (3:5)", "  fieldNoSelection (6:3)"]);
