...
import { createDevkitErrorPayload, ERROR_CODES, LspError, toDevkitErrorPayload } from "../../src/shared/errors.ts";
...
describe("shared errors payload", () => {
	...
	it("maps LspError to devkit payload", () => {
		const payload = toDevkitErrorPayload(new LspError(ERROR_CODES.LSP_ACTION_NOT_ALLOWED, "blocked"));
	...
	it("uses moduleHint for non-specialized errors and keeps LspError precedence", () => {
		...
		const lspPayload = toDevkitErrorPayload(new LspError(ERROR_CODES.LSP_TIMEOUT, "lsp timeout"), {
			moduleHint: "guards",
		});
