...
describe("MarkItDownProvider", () => {
	...
	it("converts a file by invoking the configured command and returns metadata", async () => {
		...
		const result = await provider(runner, { executable: "markitdown", args: ["--format", "md"] }).convertFile(input, {
	...
	it("throws FILE_TOO_LARGE when input exceeds maxResponseBytes before invoking the runner", async () => {
		...
		await assert.rejects(
			...
			(error: unknown) => error instanceof ConvertProviderError && error.code === CONVERT_ERROR_CODES.FILE_TOO_LARGE
		);
	...
	it("throws CONVERT_TIMEOUT when the CLI exceeds timeout", async () => {
		...
		await assert.rejects(
			...
			(error: unknown) => error instanceof ConvertProviderError && error.code === CONVERT_ERROR_CODES.CONVERT_TIMEOUT
		);
	});
	...
	it("throws CONVERT_FAILED with redacted stderr summary on non-zero exit", async () => {
		...
		await assert.rejects(
			...
			(error: unknown) =>
				...
				error.causeSummary ===
					"very bad conversion failure api_key=[REDACTED] https://example.com/doc.pdf?token=[REDACTED]"
		);
	});
