...
describe("convertContent local path", () => {
	it("rejects missing or conflicting sources", async () => {
		...
		assert.equal(getToolkitActivityLog().filter((entry) => entry.type === "convert").length, 2);
	...
	it("converts local path and applies per-call option overrides", async () => {
		...
		const entries = getToolkitActivityLog();
	...
	it("follows manual redirects and converts the final downloaded file", async () => {
		...
		try {
			const result = await convertContent({ url: "https://93.184.216.34/start.pdf" }, config, undefined, provider);

			assert.equal("error" in result, false);
			if ("error" in result) return;
			assert.equal(result.source, "https://93.184.216.34/start.pdf");
			assert.equal(provider.calls.length, 1);
			assert.deepEqual(calls, ["https://93.184.216.34/start.pdf", "https://93.184.216.34/final.pdf"]);
		} finally {
			fetchMock.mock.restore();
		}
	});
