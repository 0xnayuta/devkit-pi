...
describe("convert_content renderers", () => {
  it("renders calls for path and url inputs", () => {
    ...
    const urlCall = textOf(
      renderConvertContentCall({ url: "https://example.com/files/document.pdf" }, theme)
    );
  ...
  it("renders compact success, expanded output, partial state, and errors", () => {
    ...
    const compact = textOf(
      renderConvertContentResult(result, { expanded: false, isPartial: false }, theme)
    );
    ...
    const expanded = textOf(
      renderConvertContentResult(result, { expanded: true, isPartial: false }, theme)
    );
    assert.match(expanded, /"provider": "markitdown"/);

    const partial = textOf(
      renderConvertContentResult(result, { expanded: false, isPartial: true }, theme)
    );
