...
async function assertFetchError(params: Parameters<typeof fetchContent>[0], code?: string, config = mergeWebConfig({})) {
	const result = await fetchContent(params, config);
...
describe("fetch_content input, security, and limits", () => {
	it("returns structured errors for missing, malformed, or blocked URLs", async () => {
		assert.deepEqual(await fetchContent({}, mergeWebConfig({})), { error: { code: "INVALID_INPUT", message: "fetch_content requires url or urls" } });
	...
	it("allows private network URLs only when explicitly configured", async () => {
		...
		await assertFetchError({ url: "http://localhost:3000/api" }, undefined, mergeWebConfig({ web: { allowPrivateNetwork: false } }));

		mockFetch("local dev server content", "text/plain");
		const allowed = await assertFetchOk({ url: "http://localhost:3000/api" }, mergeWebConfig({ web: { allowPrivateNetwork: true } }));
	...
	it("limits response bytes and max content characters", async () => {
		...
		mockFetch("<html><head><title>Hello</title></head><body><script>x</script><h1>Hello</h1><p>World</p></body></html>", "text/html");
		const contentLimited = await assertFetchOk({ url: "https://93.184.216.34/page" }, mergeWebConfig({ web: { maxContentChars: 8 } }));
...
describe("fetch_content Jina reader fallback", () => {
	it("falls back for JS-heavy pages when enabled", async () => {
		...
		globalThis.fetch = ((input: string | URL) => {
			...
			if (url.startsWith("https://r.jina.ai/")) {
				return Promise.resolve(new Response("Markdown Content:\n# Better Title\n\nThis is useful extracted content from Jina.", { status: 200, headers: { "content-type": "text/plain" } }));
			}
			return Promise.resolve(new Response("<html><head><title>Stub</title></head><body><script>a</script><script>b</script><script>c</script><script>d</script><div id='app'></div></body></html>", { status: 200, headers: { "content-type": "text/html" } }));
		}) as typeof fetch;
	...
	it("honors preferReader and jinaTriggers", async () => {
		...
		globalThis.fetch = ((input: string | URL) => {
			const url = String(input);
			calls.push(url);
			if (url.startsWith("https://r.jina.ai/")) return Promise.resolve(new Response("Markdown Content:\n# Jina Title\n\nReader content.", { status: 200, headers: { "content-type": "text/plain" } }));
			return Promise.resolve(new Response("<html><head><title>Original</title></head><body><p>This page has enough normal content and should only use Jina when requested.</p></body></html>", { status: 200, headers: { "content-type": "text/html" } }));
		}) as typeof fetch;

		const preferred = await assertFetchOk({ url: "https://93.184.216.34/page", preferReader: true }, mergeWebConfig({ web: { enableJinaFallback: true } }));
		assert.equal(preferred.results[0].title, "Jina Title");
		assert.equal(calls.some((call) => call.startsWith("https://r.jina.ai/")), true);

		calls.length = 0;
		const noAutomatic = await assertFetchOk({ url: "https://93.184.216.34/js" }, mergeWebConfig({ web: { enableJinaFallback: true, jinaTriggers: [] } }));
		assert.ok(noAutomatic.results[0].content.length >= 0);
		assert.equal(calls.some((call) => call.startsWith("https://r.jina.ai/")), false);
	});
	...
	it("does not send private network URLs to Jina and caps Jina response bytes", async () => {
		...
		globalThis.fetch = ((input: string | URL) => {
			const url = String(input);
			calls.push(url);
			if (url.startsWith("https://r.jina.ai/")) return Promise.resolve(new Response("Markdown Content:\n# Jina Title\n\n" + "x".repeat(100), { status: 200, headers: { "content-type": "text/plain" } }));
			return Promise.resolve(new Response("<html><body><div id='app'></div></body></html>", { status: 200, headers: { "content-type": "text/html" } }));
		}) as typeof fetch;

		await assertFetchOk({ url: "http://localhost:3000/app", preferReader: true }, mergeWebConfig({ web: { allowPrivateNetwork: true, enableJinaFallback: true } }));
		assert.equal(calls.some((call) => call.startsWith("https://r.jina.ai/")), false);

		calls.length = 0;
		const limited = await assertFetchOk({ url: "https://93.184.216.34/page", preferReader: true }, mergeWebConfig({ web: { enableJinaFallback: true, maxResponseBytes: 35 } }));
		assert.equal(calls.some((call) => call.startsWith("https://r.jina.ai/")), true);
...
describe("fetch_content supported and unsupported content types", () => {
	it("accepts common text-friendly content types", async () => {
		...
		for (const [url, contentType, expected] of cases) {
			const body = contentType.includes("json") ? '{"key":"value","data":[]}' : contentType.includes("html") ? "<html><body>HTML content</body></html>" : contentType.includes("custom") ? "custom text content" : "Plain text content";
	...
	it("uses URL extension fallback for generic or text/plain content types", async () => {
		const cases = [
			{ url: "https://93.184.216.34/data.json", type: "application/octet-stream", body: '{"hello":"world"}', expect: /hello/, contentType: "application/json" },
			{ url: "https://93.184.216.34/docs/readme.md", type: "text/plain", body: "# Hello\n\nThis is markdown", expect: /Hello/ },
			{ url: "https://93.184.216.34/table.csv", type: "text/plain", body: "name,age\nAda,36", expect: /\| name/, contentType: "text/csv" },
			{ url: "https://93.184.216.34/feed.xml", type: "text/plain", body: "<rss><channel><title>Feed</title><item><title>One</title></item></channel></rss>", expect: /Feed: Feed/, contentType: "application/rss+xml" },
			{ url: "https://93.184.216.34/app.js", type: "application/octet-stream", body: "console.log('hello');", expect: /console\.log/ },
		];
...
describe("fetch_content extraction helpers", () => {
	...
	it("extracts heading titles and detects Jina triggers", () => {
		...
		const jsHeavy = "<html><body><script>a</script><script>b</script><script>c</script><script>d</script><div id='app'></div></body></html>";
	...
	it("extracts plain text and HTML with metadata, sanitization, and truncation", () => {
		const text = extractPlainText("https://example.com", "  hello   world  ", { maxContentChars: 1000, contentType: "text/plain" });
		...
		const html = extractHtml("https://example.com", "<html><head><title>Page Title</title></head><body><!-- secret --><script>alert('x')</script><style>.a{}</style><noscript>JS</noscript><p>hello&nbsp;world<br>next &amp; more</p></body></html>", { maxContentChars: 1000, contentType: "text/html" });
		...
		assert.equal(extractHtml("https://example.com", `<html><body>${"word ".repeat(1000)}</body></html>`, { maxContentChars: 100 }).truncated, true);
	});
});
...
describe("fetch_content handlers", () => {
	...
	it("handles HTML, plain text, and markdown", () => {
		const html = getHandler("html").process("<html><head><title>My Page</title></head><body><script>var x=1;</script><style>.a{}</style><h1>Hello</h1><p>World</p></body></html>", "https://example.com");
	...
	it("pretty-prints JSON, extracts useful JSON-LD, and falls back on invalid JSON", () => {
		...
		const bigArray = getHandler("json").process(JSON.stringify(Array.from({ length: 200 }, (_, id) => ({ id }))), "https://api.example.com/list");
		assert.match(bigArray.content, /more items/);
		assert.doesNotMatch(bigArray.content, /"id": 199/);

		const jsonLd = getHandler("json").process(JSON.stringify({ "@context": "https://schema.org", "@type": "Article", headline: "My Great Article", description: "An article", author: { name: "Alice" }, datePublished: "2024-01-15", articleBody: "Body text" }), "https://example.com/article");
	...
	it("converts CSV/TSV to markdown tables with row and column limits", () => {
		const csv = getHandler("csv").process('name,desc\nAlice,"Lives in NYC, NY"\nBob,"Simple"', "https://example.com/data.csv");
		...
		const wide = getHandler("csv").process(`${Array.from({ length: 30 }, (_, i) => `col${i}`).join(",")}\n${Array.from({ length: 30 }, (_, i) => `val${i}`).join(",")}`, "https://example.com/wide.csv");
	...
	it("handles XML/RSS/Atom/YAML and unsupported parse fallback", () => {
		const rss = getHandler("xml").process("<rss><channel><title>My Blog</title><item><title>First Post</title><link>https://example.com/post1</link><description>Interesting things.</description></item></channel></rss>", "https://example.com/feed.xml");
		assert.equal(rss.contentType, "application/rss+xml");
		assert.match(rss.content, /Feed: My Blog/);
		assert.match(rss.content, /First Post/);

		const atom = getHandler("xml").process('<feed xmlns="http://www.w3.org/2005/Atom"><title>My Atom Feed</title><entry><title>Entry One</title><link href="https://example.com/entry1"/><summary>Summary</summary></entry></feed>', "https://example.com/atom.xml");
		...
		const yaml = getHandler("yaml").process("  name: Alice\n  age: 30\n\n  city: NYC  ", "https://example.com/config.yml");
		...
		const fallback = runHandler(getHandler("unsupported"), "some data", "https://example.com/file.pdf", "unsupported");
