...
describe("web_search", () => {
	...
	it("treats no search results as a successful empty result", async () => {
		...
		const result = await webSearch({ query: "unlikely empty query" }, mergeWebConfig({ web: { provider: "ddgs" } }));
	...
	it("caps ddgs results at 5 regardless of numResults request", async () => {
		...
		globalThis.fetch = ((input: string | URL) => {
			...
			const manyLinks = Array.from(
				{ length: 10 },
				(_, i) => `<a href="https://duckduckgo.com/l/?uddg=https%3A%2F%2Fexample.com%2F${i}">Result ${i}</a>`
			).join("");
			return Promise.resolve(
...
describe("web structured errors", () => {
	...
	it("maps HTTP and network errors to structured web errors", () => {
		...
		assert.equal(
			mapNetworkErrorToWebError(new Error("fetch failed: connection refused")).code,
			WEB_ERROR_CODES.NETWORK_ERROR
		);
		assert.equal(
			mapNetworkErrorToWebError(new Error("getaddrinfo ENOTFOUND example.com")).code,
			WEB_ERROR_CODES.NETWORK_ERROR
		);
	...
	it("formats errors and summarizes retryable failures", () => {
		const formatted = formatWebError(
			createWebError(WEB_ERROR_CODES.NETWORK_ERROR, "Network error", { provider: "ddgs" })
		);
