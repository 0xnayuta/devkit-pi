...
function checkWebErrorCodes() {
	...
	const referencedCodes = [...canonicalDoc.matchAll(/`([A-Z][A-Z0-9_]*(?:_[A-Z0-9]+)*)`/g)].map((m) => m[1]);
...
function checkConfigDefaultDrift() {
	...
	for (const file of configDocs) {
		...
		const requiredSnippets = [
			...
			[
				`| \`subagents.timeoutMs\` | number | \`${defaults.subagentsTimeoutMs}\``,
				"subagents.timeoutMs table default",
			],
			[
				`| \`subagents.idleTimeoutMs\` | number | \`${defaults.subagentsIdleTimeoutMs}\``,
				"subagents.idleTimeoutMs table default",
			],
			[
				`| \`web.maxResponseBytes\` | number | \`${defaults.webMaxResponseBytes}\``,
				"web.maxResponseBytes table default",
			],
			[
				`| \`convertContent.maxContentChars\` | number | \`${defaults.convertMaxContentChars}\``,
				"convertContent.maxContentChars table default",
			],
			[`| \`lsp.hook.enabled\` | boolean | \`${defaults.lspHookEnabled}\``, "lsp.hook.enabled table default"],
			[
				`| \`lsp.hook.mode\` | \`agent_end\` / \`edit_write\` / \`disabled\` | \`${defaults.lspHookMode}\``,
				"lsp.hook.mode table default",
			],
		];
