...
async function main() {
	...
	const summary = {
		...
		totals: {
			...
			linePct: totals.coverableLines > 0 ? round2((totals.coveredLines / totals.coverableLines) * 100) : 100,
			functionPct: round2(
				fileRows.length > 0 ? fileRows.reduce((n, r) => n + r.functionPct, 0) / fileRows.length : 100
			),
		},
		byModule,
		unknownFiles,
	...
	const moduleLines = modules.map((m) => `- ${m}: ${byModule[m].linePct}% (${byModule[m].files} files)`).join("\n");
