...
- Phase 3：已闭环（状态清单、状态模型测试与后续语义补强已完成，收尾审计已归档）。
...
- Phase 6：闭环验收完成（quality gate green）。
- Phase 7：Slice 7.1（风格差距审计）已完成。
- Phase 7：Slice 7.2（formatter 策略试跑）已完成。
- Phase 7：Slice 7.3（纯格式化，不混逻辑）已完成并通过全量门禁。
- Phase 7：Slice 7.4（类型收紧）第 1~10 批已完成：LSP / convert / web / subagents / shared / commands / config / extension 已完成残留 `any` 清理或确认无类型级 `any` 残留。
- Phase 7：Slice 7.4 已闭环（本轮全量门禁 `typecheck + lint + test + docs:check` 通过）。
- Phase 7：整体已闭环（收尾审计已补齐并完成归档同步）。
- Phase 7：已冻结，仅接受回归修复，不再进行风格/类型扩改。
...
3. Phase 3：状态模型对齐 Pi session tree（已闭环）。
4. Phase 4：统一错误模型（已闭环）。
5. Phase 5：guards 分层与可选 hard gate（已闭环）。
6. Phase 6：Subagents 向官方 example 收敛（已闭环）。
7. Phase 7：工程风格与格式对齐（已闭环）。
