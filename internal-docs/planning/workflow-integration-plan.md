...
- [ ] 新增 `guards` 模块，职责清晰；
- [ ] git context notice 只显示一次；
- [ ] first write notice 只提醒一次；
- [ ] verification status notice 能检测“修改但未验证”；
- [ ] 所有 guardrails 默认只提醒，不阻止；
- [ ] 配置可关闭 guardrails；
- [ ] 不引入 Plan Tracker / phase state；
- [ ] 不复制 `pi-superpowers-plus` runtime monitor。
